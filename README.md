...
提供1V1/2V2两种多人游戏模式。多人游戏仅在同一局域网环境下可以连接。

多人游戏选择开始游戏→新建游戏，选择游戏人数。其他游戏端选择加入游戏。待连接成功的游戏人数足够后就会自动开始游戏。

创建房间的服务端会有控制台程序，请勿关闭。

可能会存在某一端长时间连接不上的情况，请在所有玩家都连接进入场景后再开始操作。否则会导致各玩家情况不一致。

如果长时间连接不上，请关闭控制台后重新进行创建。
...
每个玩家进入游戏后会跳转至自己的领地，只有在自己的领地才能进行建造。
...
左下角的小地图会显示地图各方单位的现状，点击小地图可以跳转屏幕至地图点击区域。Tab键可以隐藏或显示小地图。
...
由于延迟的原因，会存在长时间游戏后各玩家情况不一致、游戏情况与预期不符等，技术能力不足十分抱歉。
